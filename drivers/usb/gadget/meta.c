...
static void android_work(struct work_struct *data)
{
	...
	if (uevent_envp) {
		...
		pr_notice("%s: not send uevent (%d %d %p),is_hwconnected=%d\n",
			 __func__, dev->connected, dev->sw_connected,
			cdev->config, is_hwconnected);
	}
...
void enable_meta_vcom(int mode)
{
	...
	#ifndef VENDOR_EDIT
	/*xing.xiong@BSP.Kernel.Debug, 2018/11/13, Modify for oppo meta vid&pid*/
